...
動機はAnkiのアドオン作りでPython書きの行き詰りによる萎え落ちを減らすことです。あと線形代数で追試くらったことです（単位はくれました）。雪辱を果たしに。
...
\section{簡約化とは}
簡約化は、連立方程式を解く際に行列の値を都合よくする操作です。実際中学からの連立方程式の解き方まんまで、式同士の足し算、引き算、掛け算をします。
例えば、３つの式を上から①②③として、
...
\section{計画}
今回はこれをPythonで実装します。しかし、上記の説明のまま実装しようとすると割と苦労します。例えば5.で繰り返しの操作がありますが、コードで書くには行数を文字で置いたりして一般化する必要があります。そこで、以下のように書き換えます。\\
\hrulefill
\begin{verbatim}
行列を縦I、横Jの大きさとする。
1.左の列から右の列へ、k行目（はじめk=1）から順に0でない数を探す。
  はじめに見つかった数をnとする。
2.nがある行をk行目と入れ替える。
3.k行目をnで割る。
4.k行目以外の行からk行目を何倍かして引き、nと同じ列のn以外の数を0にする。
5.kに1を加え、1.からやり直す。k>Iとなるか、
  1.で0でない数が見つからなくなったとき、計算を終了する。
\end{verbatim}
\hrulefill\\
使いやすい文字をいくつか入れてみました。いっぺんに実装を考えるときついですが、順番ごとに考えるとなんかいけそうな気がします。こっからPythonを書いていきましょう！
\section{実装}
\subsection{Pythonでの行列}
\subsection{リスト内包表記}
Pythonで行列のような二次元リストを扱う際、リスト内包表記が便利です。リストの中にリストを入れる書き方です。
例えば$\begin{cases}
        x + 2y - z = 2  \\
        2x - y + 3z = 9 \\
        3x + y + z = 8
...
また、i行j列目、すなわち$a_ij$は
今回はこの行列aを演算対象とします。
\subsection{numpy}
他にはnumpyというライブラリを使う方法もあります。こちらもリスト内包表記に似ていますが、こちらは行列自体を扱うことができます。更に簡約化はもちろん逆行列、行列式も1行で求めてくれます。よって今回の簡約化実装は車輪の最発明となります。
